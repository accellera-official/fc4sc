...
TEST(cvp_get_bin_hit_count_test, basic) {
  ...
  cvp_get_bin_hit_count_test cvg("cvg",__FILE__,__LINE__,cntxt);
...
xml_printer::coverage_save("basic_bin_hit_" + std::string(::testing::UnitTest::GetInstance()->current_test_info()->name()) + ".xml",cntxt);
